...
let env = 'subdomains-lite';
let hostname = 'epfl.ch';
...
async function fetchAndFilterSites() {
	try {
		...
		filteredSites = filteredSites.filter((site) => site.openshiftEnv == env && site.wpInfra);
...
const determinePlugins = (categories, openshiftEnv) => {
	if (openshiftEnv == 'subdomains-lite' || openshiftEnv == 'subdomains') {
...
metadata:
  ...
  namespace: ${namespace}
spec:
  hostname: ${(env == 'subdomains-lite' ? getName(site.ansibleHost) : 'www') + '.' + hostname}
  path: ${env == 'subdomains-lite' ? '/' : path}
  owner:
    epfl:
      unitId: ${site.unitId}
  wordpress:
